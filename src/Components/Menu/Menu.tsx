...
const Menu = () => {
    ...
    return (
      <div className="py-20 relative overflow-hidden">
            <Image alt=""  className="absolute top-15 left-1/2" src={CORE}/>
            <Image alt=""  className="absolute bottom-8 left-2/3" src={MIC}/>
            <Image alt=""  className="absolute bottom-14  right-2/3" src={VIDEO}/>
            <div className="">
                <div className="py-10 flex flex-col space-y-5 items-center">
                    <div className="flex gap-4">
                    {
                        menu1.map((s,index)=><div className={`${colors[index % colors.length]} px-10 py-2 rounded-md  flex-shrink-0 cursor-pointer`}>{s}</div>)
                    }
                    </div>
                    <div className="flex gap-5">
                        {
                            menu2.map((s,index)=><div className={`${colors[index % colors.length]} px-10 py-2 rounded-md  flex-shrink-0 cursor-pointer`}>{s}</div>)
                        }
                    </div>
                </div>
            </div>
